/obj/item/weapon/pickaxe/heavydrill
	name = "heavy duty"
	desc = "Vroom vroom."
...
/obj/item/weapon/pickaxe/heavydrill/New()
...
/obj/item/weapon/pickaxe/heavydrill/Destroy()
	processing_objects -= src
	if(reagents)
		qdel(reagents)
	..()

/obj/item/weapon/pickaxe/heavydrill/proc/turnOn(mob/user as mob)
	if(on) return
...
/obj/item/weapon/pickaxe/heavydrill/proc/turnOff(mob/user as mob)
	if(!on) return
...
/obj/item/weapon/pickaxe/heavydrill/attack_self(mob/user as mob)
	if(!on)
		turnOn(user)
	else
		turnOff(user)

/obj/item/weapon/pickaxe/heavydrill/afterattack(atom/A as mob|obj|turf|area, mob/user as mob, proximity)
	if(!proximity) return
	..()
	if(on)
...
/obj/item/weapon/pickaxe/heavydrill/process()
	if(!on) return
...
/obj/item/weapon/pickaxe/heavydrill/proc/get_fuel()
	return reagents.get_reagent_amount("fuel")

/obj/item/weapon/pickaxe/heavydrill/examine(mob/user)
	if(..(user,0))
		if(max_fuel)
			to_chat(usr, "<span class = 'notice'>The [src] feels like it contains roughtly [get_fuel()] units of fuel left.</span>")

/obj/item/weapon/pickaxe/heavydrill/suicide_act(mob/user)
...
/obj/item/weapon/pickaxe/heavydrill/update_icon()
	if(on)
