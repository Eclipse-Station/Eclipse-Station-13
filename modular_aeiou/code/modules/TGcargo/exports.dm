...
//	if(!exports_list)
//		exports_list = generate_chameleon_choices(/obj/item/clothing/glasses, list(src.type))

/datum/export/Destroy()
	qdel()
	return ..()

/datum/export/proc/process()
	..()
...
/datum/export/proc/get_cost(obj/O, contr = 0)
	var/amount = get_amount(O, contr)
	if(k_elasticity!=0)
		return round((cost/k_elasticity) * (1 - NUM_E**(-1 * k_elasticity * amount)))	//anti-derivative of the marginal cost function
	else
		return round(cost * amount)	//alternative form derived from L'Hopital to avoid division by 0
...
/datum/export/proc/get_amount(obj/O, contr = 0)
	return 1
...
/datum/export/proc/applies_to(obj/O, contr = 0)
	if(contraband && !contr)
		return FALSE
	if(!include_subtypes && !(O.type in export_types))
		return FALSE
...
/datum/export/proc/sell_object(obj/O, contr = 0)
...
/datum/export/proc/total_printout(contr = 0)
