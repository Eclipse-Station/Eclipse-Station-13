...
/obj/item/weapon/handheld_recharger/examine(mob/user)
	if(!..(user, 1))
		return

	if(bcell)
		user <<"<span class='notice'>The [src] has a cell.</span>"
	if(!bcell)
		user <<"<span class='warning'>The [src] does not have a power cell installed.</span>"
