...
/obj/machinery/space_cooler/New()
	..()
	if(cell_type)
		cell = new cell_type(src)
	update_icon()

/obj/machinery/space_cooler/update_icon()
	overlays.Cut()
...
/obj/machinery/space_cooler/examine(mob/user)
	..(user)

	user << "The cooler is [on ? "on" : "off"] and the hatch is [panel_open ? "open" : "closed"]."
	if(panel_open)
...
/obj/machinery/space_cooler/powered()
	if(cell && cell.charge)
		return 1
	return 0

/obj/machinery/space_cooler/emp_act(severity)
...
/obj/machinery/space_cooler/attackby(obj/item/I, mob/user)
...
/obj/machinery/space_cooler/attack_hand(mob/user as mob)
	add_fingerprint(user)
	interact(user)

/obj/machinery/space_cooler/interact(mob/user as mob)
...
/obj/machinery/space_cooler/Topic(href, href_list)
	if(usr.stat)
		return
...
/obj/machinery/space_cooler/process()
	if(on)
		if(cell && cell.charge)
