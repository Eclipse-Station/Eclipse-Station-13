...
/obj/machinery/door/airlock/proc/hack(mob/user as mob)
	if(src.aiHacking==0)
		...
		spawn(20)
			...
			sleep(50)
			if(src.canAIControl())
				to_chat(user,"Alert canceled. Airlock control has been restored without our assistance.")
				src.aiHacking=0
				return
...
/obj/machinery/door/airlock/open(var/forced=0)
	...
	if(arePowerSystemsOn())
		playsound(src.loc, open_sound_powered, 40, 1)
	else
...
/obj/machinery/door/airlock/close(var/forced=0)
	...
	has_beeped = 0
	if(arePowerSystemsOn())
		playsound(src.loc, close_sound_powered, 40, 1)
	else
		playsound(src.loc, open_sound_unpowered, 65, 1)
	for(var/turf/turf in locs)
