...
/mob/living/carbon/human/handle_autohiss(message, datum/language/L)
	if(!client || autohiss_mode == AUTOHISS_OFF) // no need to process if there's no client or they have autohiss off //Eclipse Edit: autohiss_mode is now a human mob var.
		return message
	return species.handle_autohiss(message, L, autohiss_mode) //Eclipse Edit: autohiss_mode is now a human mob var.

/mob/living/carbon/human //Eclipse Edit: Instead of /client - Fixed autohiss being lost on disconnect.
	var/autohiss_mode = AUTOHISS_OFF

/mob/living/carbon/human/verb/toggle_autohiss() //Eclipse Edit: Instead of /client - Autohiss fix.
