/obj/item/weapon/deadringer
	...
	icon = 'icons/obj/anomalous/deadringer.dmi'
...
/obj/item/weapon/deadringer/attack_self(var/mob/living/user as mob)
	...
	if(!activated)
		if(timer == 0)
			H << "<font color='blue'>You press a small button on [src]'s side. It starts to hum quietly.</font>"
			bruteloss_prev = H.getBruteLoss()
			fireloss_prev = H.getFireLoss()
			activated = 1
			return
		else
			H << "<font color='blue'>You press a small button on [src]'s side. It buzzes a little.</font>"
			return
	if(activated)
		H << "<font color='blue'>You press a small button on [src]'s side. It stopped humming.</font>"
		activated = 0
		return
	...
	if(activated)
		if (ismob(src.loc))
			...
			if(H.getBruteLoss() > bruteloss_prev || H.getFireLoss() > fireloss_prev)
				activated = 0
				watchowner << "<font color='blue'>You fade into nothingness, leaving behind a fake body!</font>"
...
/obj/item/weapon/deadringer/proc/deathprevent()
	for(var/mob/living/simple_animal/D in range(7, src))
		D.LoseTarget()
...
/obj/item/weapon/deadringer/proc/uncloak()
	if(watchowner)
		...
		playsound(get_turf(src), 'sound/magic/uncloak.ogg', 50, 1, -1)
	return
